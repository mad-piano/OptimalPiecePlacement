...
class StockfishEngine:
    ...
    def track_piece_move(self, games=1, depth=1, fen="", initial_fen="", move_limit=None):
        ...
        for _ in range(games):
            ...
            while not (
                ...
                best_move = self.find_best_move(fen, random.randint(10, depth))
                ...
                if move_limit is not None: 
                    move_limit -= 0.5  # Adjust the move_limit decrement to float type
                fen = self.board.fen()
    ...
    @staticmethod
    def plot_ranking_pie_charts(white_ranking, black_ranking, total_games, total_moves, opening_name, depth):
        fig, axes = plt.subplots(2, len(white_ranking), figsize=(15, 8))
        fig.suptitle(f'{opening_name}: Piece Movement Frequency (Total Games: {total_games} | Total moves per game: {total_moves})', fontsize=16)
        ...
        for i, (piece, locations) in enumerate(white_ranking):
            if locations:
                ...
                axes[0, i].pie(
                    ...
                    startangle=140, 
                    textprops={'fontsize': 8},  # Slightly larger square name
                    colors=plt.cm.Blues([0.3 + 0.7 * i / len(white_ranking) for _ in range(len(squares))])
                )
                axes[0, i].set_title(f'White {piece}', fontsize=16)
            else:
                axes[0, i].axis('off')  # Hide pie chart if no locations available
        ...
        for i, (piece, locations) in enumerate(black_ranking):
            if locations:
                ...
                axes[1, i].set_title(f'Black {piece}', fontsize=16)
            else:
                axes[1, i].axis('off')  # Hide pie chart if no locations available
